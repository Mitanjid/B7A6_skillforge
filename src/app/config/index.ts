...
const config = {
  ...
  platform_publish_base_fee:
    Number(process.env.PLATFORM_PUBLISH_BASE_FEE) || 500,
  platform_publish_fee_per_mark:
    Number(process.env.PLATFORM_PUBLISH_FEE_PER_MARK) || 2,

  invitation_expiry_days: Number(process.env.INVITATION_EXPIRY_DAYS) || 7,
  attempt_sweep_interval_ms:
    Number(process.env.ATTEMPT_SWEEP_INTERVAL_MS) || 60_000,
