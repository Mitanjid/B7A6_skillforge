  ...
  for (const email of uniqueEmails) {
    ...
    if (existing) {
      ...
      await prisma.invitation.update({
        ...
        data: { candidateId, token, expiresAt, status: InvitationStatus.PENDING },
      });
      results.push({ email, outcome: "reinvited", linkedToAccount: !!candidateId });
    } else {
      await prisma.invitation.create({
        data: { assessmentId, email, candidateId, token, expiresAt },
      });
      results.push({ email, outcome: "invited", linkedToAccount: !!candidateId });
    }
...
const getMyInvitations = async (
  ...
  await prisma.invitation.updateMany({
    where: { ...where, status: InvitationStatus.PENDING, expiresAt: { lt: new Date() } },
...
const getInvitationPreview = async (token: string) => {
  ...
  if (status === InvitationStatus.PENDING && invitation.expiresAt < new Date()) {
    await prisma.invitation.update({
