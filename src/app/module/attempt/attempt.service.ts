...
const startAttempt = async (
	...
	const deadline = new Date(
		attempt.startedAt.getTime() +
			invitation.assessment.durationMinutes * 60_000,
	);
		...
		async (tx) => {
			for (const ap of assessmentProblems) {
				...
				if (ap.problem.type === ProblemType.MCQ) {
					const isCorrect =
						!!submission &&
						submission.selectedOption === ap.problem.correctOption;
...
const submitAnswer = async (
	...
	if (isMcq && !payload.selectedOption) {
		throw new AppError(
			httpStatus.BAD_REQUEST,
			"selectedOption is required for this problem",
		);
	}
	if (!isMcq && !payload.answerText) {
		throw new AppError(
			httpStatus.BAD_REQUEST,
			"answerText is required for this problem",
		);
	}
...
const sweepExpiredAttempts = async () => {
	...
	for (const attempt of expired) {
		try {
			...
			console.error(
				`Failed to auto-submit expired attempt ${attempt.id}:`,
				err,
			);
		}
	}
