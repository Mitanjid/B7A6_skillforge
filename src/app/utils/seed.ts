...
import { AuthProvider, Difficulty, ProblemType, Role } from '../../../generated/prisma/client.js';
import { prisma } from '../lib/prisma.js';

const seedAdmin = async () => {
  if (!config.admin_email || !config.admin_password) {
    console.warn('[seed] Skipping admin — ADMIN_EMAIL/ADMIN_PASSWORD not set in .env');
    return;
  }
...
const seedTesterCandidate = async () => {
  if (!config.tester_candidate_email || !config.tester_candidate_password) return;
  ...
  console.log(`[seed] Demo candidate created: ${config.tester_candidate_email}`);
};
    ...
    data: [
      {
        ...
        description: 'Choose the correct behavior of the strict equality operator.',
      ...
      },
      {
        ...
        description: 'Briefly describe how you would design a rate limiter for a public API.',
        type: ProblemType.WRITTEN,
